...
def main():
    ...
    if args.dataset_name is not None:
        ...
        df_train = pd.read_csv("/root/histology_lib/data/train.csv")
        target_name = "Target 1 - Гистологический класс"
        d = {
            "Пилоидная астроцитома": 0,
            "Диффузная астроцитома": 1,
            "Олигоастроцитома": 2,
            "Анапластическая астроцитома": 3,
            "Глиобластома": 4,
            "Плеоморфная ксантоастроцитома": 5,
            "Олигодендроглиома": 6,
        ...
        for _, i in tqdm(df_train.iterrows(), total=df_train.shape[0]):
            ...
            train_target.extend([d_ru2en[i[target_name]]] * len(images))

        dataset = Dataset.from_dict({'train': {'image': train_data, 'text': train_target}})
        # data_files = {}
